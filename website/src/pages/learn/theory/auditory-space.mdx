...
<MiniRepl client:visible tune={`sound("hh*16").pan(sine.range(0.2, 0.8).slow(2))`} />

The hi-hats glide smoothly from left to right and back. This creates movement and interest - the ear follows the sound across the stereo field.
...
Here's a mono pattern:

<MiniRepl client:visible tune={`n("0 2 4 7").scale("C:minor").s("sawtooth").lpf(800)`} />

Now add `.jux(rev)`:

<MiniRepl client:visible tune={`n("0 2 4 7").scale("C:minor").s("sawtooth").lpf(800).jux(rev)`} />

The sound explodes into stereo. What happened?
...
**Control the width with juxBy:**

<MiniRepl client:visible tune={`n("0 2 4 7").scale("C:minor").s("sawtooth").lpf(800)
  .juxBy("<0 0.25 0.5 0.75 1>", rev)`} />

`0` = mono (no stereo split), `1` = full width. Find your sweet spot.
...
Play only the first line:

<MiniRepl client:visible tune={`$: sound("bd sd bd sd").room(0.01)
// $: sound("bd sd bd sd").room(0.8).roomsize(4).gain(0.5)`} />

Now comment out the first line and uncomment the second:

<MiniRepl client:visible tune={`// $: sound("bd sd bd sd").room(0.01)
$: sound("bd sd bd sd").room(0.8).roomsize(4).gain(0.5)`} />

The same drums, but one feels like it's right in front of you while the other sounds like it's across a large hall. This is **reverb** - the collection of echoes that occur when sound bounces off surfaces in a space.
...
<MiniRepl client:visible tune={`// Small room (tight, intimate - a bathroom or closet)
note("c4 eb4 g4").s("gm_vibraphone")
  .room(0.4).roomsize(0.5).roomlp(6000)`} />

<MiniRepl client:visible tune={`// Large hall (expansive, dramatic - a cathedral)
note("c4 eb4 g4").s("gm_vibraphone")
  .room(0.6).roomsize(8).roomlp(3000)`} />
...
<MiniRepl client:visible tune={`sound("rim").delay(0.5).delaytime(0.25).delayfeedback(0.4)`} />
...
<MiniRepl client:visible tune={`stack(
  // Front: drums (dry, present)
  sound("bd [~ bd] sd [bd sd]").room(0.05),

  // Middle: bass (slightly back)
  note("c2 ~ eb2 ~").s("sawtooth").lpf(500)
    .room(0.15).roomsize(2),

  // Back: pad (distant, diffuse)
  note("<c3,eb3,g3>").s("triangle").slow(2)
    .room(0.6).roomsize(6).lpf(2000)
    .delay(0.2).delaytime(0.333).gain(0.4)
).cpm(30)`} />

Notice the **depth staging**: drums upfront and dry, bass slightly back, pad far in the distance. Each element occupies a different front-to-back position.
...
<MiniRepl client:visible tune={`$: note("c2").s("sawtooth").lpf(800)
$: note("c3 eb3 g3").s("sawtooth").lpf(800)`} />

The bass and chords are fighting. They both occupy similar frequency ranges and blur together. Now listen:

<MiniRepl client:visible tune={`$: note("c2").s("sawtooth").lpf(400)
$: note("c3 eb3 g3").s("sawtooth").hpf(300).lpf(2000)`} />

By carving out separate frequency spaces - bass gets the lows (under 400Hz), chords get the mids (300Hz-2000Hz) - each element becomes distinct. This is **subtractive EQ** - removing frequencies to create space.
...
This is the classic acid house sound - a resonant low-pass filter sweep. The high `.lpq(12)` value makes the filter "sing" at its cutoff point.
