...
<MiniRepl client:idle tune={`chord("C").voicing().s("piano")`} punchcard punchcardLabels={false} />

That's your foundation. One chord, repeating. Now let's make it interesting.

**Add a second chord for contrast:**

<MiniRepl client:idle tune={`chord("<C F>").voicing().s("piano")`} punchcard punchcardLabels={false} />

Listen to the movement. C → F creates forward motion. The F wants to go somewhere—it has energy.

**Try different second chords:**

<MiniRepl client:idle tune={`chord("<C G>").voicing().s("piano")`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`chord("<C F G>").voicing().s("piano")`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`chord("<C G F>").voicing().s("piano")`} punchcard punchcardLabels={false} />

C → G → F has a different arc. The tension (G) comes earlier.

<MiniRepl client:idle tune={`chord("<F C G>").voicing().s("piano")`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`chord("<C F G C>").struct("x@3 x").voicing().s("piano")`} punchcard punchcardLabels={false} />
...
C → D → E → F moves by steps. It feels flowing, connected.

**Jump movement (strong):**

<MiniRepl client:idle tune={`chord("<C F C F>").voicing().s("piano")`} punchcard punchcardLabels={false} />

C → F is a 4th leap. Strong, decisive movement.

**Large leaps (dramatic):**

<MiniRepl client:idle tune={`chord("<C F# C F#>").voicing().s("piano")`} punchcard punchcardLabels={false} />

C → F# is a tritone (half the octave). Maximum tension, very dramatic.
...
<MiniRepl client:idle tune={`chord("<C C/E C/G C>").voicing().s("piano")`} punchcard punchcardLabels={false} />

The chords change color, but all share the C bass note. Creates cohesion.

**Static chords, moving bass:**

<MiniRepl client:idle tune={`chord("<C>").struct("x").voicing().stack(
  x => x.voicing(),
  x => x.rootNotes("<2 3 4 3>").note().s('sawtooth').lpf(600)
...
<MiniRepl client:idle tune={`chord("<C@3 ~ F@3 ~ G@3 ~ C@4>").struct("x").voicing().s("piano").cpm(20)`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`stack(
  chord("<C Am F G>").struct("~ x ~ x").voicing(),
  sound("bd ~ bd ~"),
  sound("~ sd ~ sd")
)`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`chord("<C^7 Eb^7 Ab^7 Db^7>").layer(
  x => x.anchor("c5").struct("[~ x]*2").voicing(),
...
<MiniRepl client:idle tune={`chord("<C^11>").voicing().s("piano")`} punchcard punchcardLabels={false} />

The 11th (F) clashes with the 3rd (E) in major chords. Sounds muddy.
...
<MiniRepl client:idle tune={`irand(12).scale('C minor').note().s('piano')`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`"[c d e f g a b]".segment(4).pick().note().s('piano')`} punchcard punchcardLabels={false} />

Picks notes from the segmented chunk.

**`.slice()` - divide and select:**

<MiniRepl client:idle tune={`"c d e f g a b c".slice(8).segment(4).note().s('piano')`} punchcard punchcardLabels={false} />
...
<MiniRepl client:idle tune={`irand(8).scale('D dorian').note().s('piano')`} punchcard punchcardLabels={false} />

Random numbers → scale degrees in D Dorian.

**`.add()` - add interval:**

<MiniRepl client:idle tune={`"c e g".choose().add("<0 7 12>").note().s('piano')`} punchcard punchcardLabels={false} />

Pick a note, add octave/fifth/unison.
...
<MiniRepl client:idle tune={`chord("<Dm7 G7 C^7 F^7 Dm7 G7 C^7@2>").layer(
  x => x.anchor("c5").struct("[~ x]*2").voicing().lpf(800),
  x => x.rootNotes(2).note().s('sawtooth').lpf(600),
  x => n("0 1 2 1").chord(x).voicing().note().s('piano')
...
**Expand Your Toolkit:**
- **[Euclidean Rhythms](/learn/euclidean-101/)** — Advanced rhythmic composition
